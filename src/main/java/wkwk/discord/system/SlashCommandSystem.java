                                ...
                                if (interaction.getOptionByIndex(0).isPresent()) {
                                    ...
                                    if ("delete".equals(subCommandGroup.getName())) {
                                        ...
                                        if (subCommandGroup.getOptionStringValueByName("title").isPresent() && subCommandGroup.getOptionStringValueByName("emoji-1").isPresent()) {
                                            ...
                                            voteDataRecord.setAllVotes(0L);
                                            if (subCommandGroup.getOptionStringValueByName("description").isPresent())
                                                voteDataRecord.setDescription(subCommandGroup.getOptionStringValueByName("description").get());
                                            for (int i = 1; i < 23; i++) {
                                                if (subCommandGroup.getOptionStringValueByName("emoji-" + i).isPresent()) {
                                                    String[] options = subCommandGroup.getOptionStringValueByName("emoji-" + i).get().split(",");
                                                    VoteRecord voteRecord = new VoteRecord();
                                                    for (char emoji : options[0].toCharArray()) {
                                                        if (EmojiManager.isEmoji(String.valueOf(emoji))) {
                                                            voteRecord.setEmojiType("unicode");
                                                            voteRecord.setEmojiText(String.valueOf(emoji));
                                                            voteRecord.setVotes(0L);
                                                            voteRecord.setVoteText(options[1]);
                                                            voteRecords.add(voteRecord);
                                                        }
                                                    }
                                                    if (voteRecord.getEmojiText().isEmpty()) {
                                                        interaction.createImmediateResponder().setFlags(MessageFlag.EPHEMERAL).setContent(options[0] + "は絵文字ではありません。").respond();
                                                        noMiss = false;
                                                        break;
                                                    }
                                                }
                                            ...
                                            if (noMiss) {
                                                voteDataRecord.setVoteRecords(voteRecords);
                                                EmbedBuilder embedBuilder = new EmbedBuilder()
                                                        .setColor(Color.CYAN)
                                                        .setAuthor(sendUser)
                                                        .setTitle(voteDataRecord.getTitle());
                                                if (!voteDataRecord.getDescription().isEmpty())
                                                    embedBuilder.setDescription(voteDataRecord.getDescription());
                                                Message voteMessage = new MessageBuilder().setEmbed(embedBuilder).send(interaction.getChannel().get()).join();
                                                voteDataRecord.setMessageId(voteMessage.getId());
                                                for (VoteRecord record : voteDataRecord.getVoteRecords()) {
                                                    voteMessage.addReaction(record.getEmojiText());
                                                }
